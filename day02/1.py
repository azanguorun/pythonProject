...
def readAction_0_Time():
    with open('./b站/0_b站执行日期.csv', mode='r', encoding='utf-8') as f:
...
def get_if_csv(mblog_url):
    ...
    with open(f'./b站/0_b站抽奖池.csv', mode='r', newline='', encoding='utf-8') as file:
...
#最终整理文件
def sort_0_csv():
    data = pd.read_csv(f'./b站/0_b站抽奖池.csv')
    sort_data = data.sort_values('抽奖时间戳', ascending=True)
    sort_data.to_csv(f'./b站/0_b站抽奖池.csv', index=False)

    frame = pd.read_csv('./b站/0_b站抽奖池.csv', engine='python', encoding='utf-8')
    ...
    data = pd.read_csv('./b站/0_b站抽奖池.csv', encoding='utf-8')
    # 过滤掉包含指定 upid 的行
    ...
    cs=pd.read_csv(f'./b站/0_b站抽奖池.csv').shape[0]
    ...
    print(sc)
    with open('./b站/0_b站执行日期.csv', mode='a', encoding='utf-8', newline='') as f:
        f.write(sc)
        f.write('\n')
...
#获取抽奖数据
def get_data():
    with open(f'./b站/0_b站抽奖池.csv', 'w', newline='', encoding="utf-8") as f:
...
def goto_at(threadName,offset, host_mid, SESSDATA, csrf):
    ...
    try:
        ...
        for item in items:
            ...
            with open(f'./b站/0_b站结果.csv', mode='a', encoding='utf-8', newline='') as f:
                csv_write = csv.writer(f)
                csv_write.writerow(data)
    except:
