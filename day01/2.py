...
def readFocusFile():
    with open(f'I:/'+fl+'.json', mode='r', encoding='utf-8') as f:
        data_json = json.load(f)
        last_time=data_json['ts']
        data=data_json['data']
        print(last_time)
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(last_time))) #今日日期
)
        # for da in tqdm(data):
        #     user_name=da['name']
        #     user_id=da['value']
        ...
        #     ups2 = ups[1]
        #     ups3 = ups[2]
        #     thread1 = Thread(target=Filedynamic, args=("thread_1", ups1,last_time, user_id, user_name, offset))  # 线程1：执行任务打印4个a
        #     thread2 = Thread(target=Filedynamic, args=("thread_2", ups2,last_time, user_id, user_name, offset))  # 线程2：执行任务打印2个b
        #     thread3 = Thread(target=Filedynamic, args=("thread_3", ups3,last_time, user_id, user_name, offset))  # 线程2：执行任务打印2个b
        #     thread1.start()  # 线程1开始
        #     thread2.start()  # 线程2开始 3_1415819751_azanguorun_b站执行日期.csv
        #     thread3.start()  # 线程2开始 3_1415819751_azanguorun_b站执行日期.csv
        #     thread1.join()  # 等待线程1结束
        #     thread2.join()  # 等待线程2结束
        #     thread3.join()  # 等待线程2结束
        #     # Filedynamic(last_time, user_id, user_name, offset)
        global ss
        ss = 1
...
#采集视频
def Filedynamic(threadName,up,last_time,user_id, user_name,offset):
    csrf = up['csrf']
    ...
    try:
        if '-352' != resp.json()['message']:
            ...
            for item in items:
                ...
                # mblog_timestamp = item['modules']['module_author']['pub_ts']  # 发布时间
                ...
                if mblog_type == 'DYNAMIC_TYPE_AV' and int(last_time) <= pub_ts:
                    ...
                    if text == '投稿视频':
                        # if int(last_time) <= mblog_timestamp:
                        mblog_url = 'https://t.bilibili.com/' + id_str  # 动态的链接
                        # exit()
                        MyDynamic(threadName,aid, name,mblog_url, title)
                        data = [mblog_url, name, title]
                        writefile(data)

            if int(last_time) <= pub_ts:
                print(threadName,'读取下一页')
                Filedynamic(threadName,up,last_time, user_id, user_name, offset)
        else:
...
#添加稍后观看
def MyDynamic(threadName,aid, name,mblog_url, title):
    # time.sleep(4)
    headers = {
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8",
...
def writefile(data):
    ...
    with open('I:/b站视频.csv', mode='a', encoding='utf-8', newline='') as f:
        # 创建一个csv写入器对象
        csv_write = csv.writer(f)
        # 将数据写入csv文件的当前行
        csv_write.writerow(data)

def get_json_data(last_time,fl):
    with open(f'I:/'+fl+'.json', 'r', encoding='utf-8') as f:  # 使用只读模型，并定义名称为f
    ...
    with open(f'I:/'+fl+'.json', 'w', encoding='utf-8') as r:
        # 将dict写入名称为r的文件中
